...
function Trending({ type, list }) {
  const sectionTitle = type === "movie" ? "Trending Movies" : "Trending TV Shows";
        ...
        <Row className="gx-3">
          {list && list.length > 0 ? (
            list.slice(0, 6).map((item) => (
              ...
              >
                <MovieCard title={item.title || item.name} path={item.poster_path} />
              </Col>
            ))
          ) : (
            <p>No trending {type === "movie" ? "movies" : "TV shows"} available.</p>
          )}
        </Row>
      </div>
      {modalShow && (
